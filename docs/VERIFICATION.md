...
## 2026-09-23 v0.5 — agent full capability + skills system (R0→R5)

- Full workflow rerun with per-stage subagents and artifacts: R0 `docs/specs/agent-capabilities.md` (§1–11) → R1 §12 (skills page 5-state wireframes, copy table, selector contract) → R2 §13 (all contracts; decision register D1–D9 closed: second `creatoros-app` server, bridge gets read-only quartet only, cron caliber unified strict) → R3 two batches → R4 AC reconciliation +7 cases → R5 sweep (v0.4's 4 polish items closed, dead CSS removed).
- Agent tool surface: 10 new tools (job/content/account/skill lists + create/toggle/update/skill_create + job_delete). Fence constants moved to zero-import `agentPolicy.ts` (allowedTools lists BOTH servers explicitly — the v0.3.0–v0.3.2 bypassPermissions lesson); PreToolUse matcher extended; job_delete gets a warn gate in the hook pass-through branch; SYSTEM_APPEND gains the anti-injection rule (webpage text must never become skill template content).
- R3 batch-1 caught a real spec bug before it shipped: R2's literal validateJobInput killed legacy `browser.navigate`/`demo` payloads — the automation-delete e2e caught it; fixed to strict-cron-for-all + payload checks scoped to agent.run.
- Cron caliber unified: `isSupportedCron`/`nextRunAt`/`CRON_ERROR_HINT` in shared/cronNext.ts, Scheduler.createJob is the single authority; IPC and Gateway inherit it. UI preview and agent-created jobs now share one expression set.
- Skills: idempotent DDL + injectable SkillsRepo (`Pick<DatabaseSync,'prepare'>` factory — vitest imports it without Electron), cron binding by skillId reference (template updates propagate; dangling skillId fails jobs loudly with the shared error copy).
- Bridge: build logic moved to `src/main/mcp/bridgeServer.ts` (fetch-injected, testable), script is a thin shell; read-only quartet registered from the single APP_TOOL_DEFS source (zod shape instance-identity asserted with `toBe`); write/destructive tools confirmed absent. Dead `src/main/mcp/tools.ts` deleted.
- vitest **149** (+57 over v0.4): skills repo CRUD/idempotency, 10 tool handlers with injected mocks, policy anchors (server names, DISALLOWED never matches MCP), bridge registration, cron additions. playwright **52** (+13): skills-page (8: CRUD, two-step delete, chips fill-not-send, run E2E, origin badge), skill-cron (3: binding roundtrip, template propagation, dangling failure + warning pill), gateway-quartet (read-only shape + 405/404 boundary), cron-agent 400 group extended. Fake-mode blind spot honestly documented: real tool invocation is manual-verify only (AC-S6c/S9 steps in the test report).
- R5 sweep: v0.4's four polish items (card sub-line, logs meta clamp, chip sizing, running-state weight) fixed and screenshot-verified; `.link-btn`/`.primary` dead CSS removed (zero spec impact, 52 green); AutomationPage/SkillsPage disabled buttons got explanatory titles; walked-but-not-fixed items registered in spec §14 (Gateway/stepTranslator legacy `any`, 5 sleep(50) race points in new specs).
- Gate: `npm run gate` exit 0 (ESLint 0 errors, 42 legacy warnings, 4×typecheck, build, vitest 149, e2e 52). Zero new dependencies. Version 0.5.0.

## 2026-09-23 v0.5 — agent full-capability tools + skills system (R0→R4)

- Flow per the mandated workflow: R0 `docs/specs/agent-capabilities.md` (source-verified anchors; R2 §13.10 corrected the tool count to 10 and the vitest-import mechanism) → R1 §12 UI spec → R2 §13 architecture (skillsRepo injected-handle form, appToolDefs single-source, D5 bridge read-only quartet, D6 strict-cron convergence at one Scheduler gate) → R3 two-batch implementation (3578a3d + a4880ed) → R4 dual-layer report `docs/specs/agent-capabilities-test-report.md`.
- Coverage of the spec's ten acceptance criteria: AC-S1~S8/S10 automated (gap analysis found zero-missing on S1/S3/S4/S6a/S6b — no duplicate tests written to pad counts), AC-S6c/S9 manual-only (fake transport never spawns the tool host; reproduction steps recorded in the R4 report §四).
- vitest **14 files / 149 tests** (was 146): +skills-repo (R3), +app-tools 31 it — all 10 tool handlers direct-tested with injected fakes, agentPolicy constants asserted, +bridge 8 it (20-tool exact surface, write-tool negative assertions, zod-instance identity with APP_TOOL_DEFS), +cron-preview 3 states.
- playwright **12 specs / 52 tests** (was 48): +skills-page 8 (page CRUD round-trip to SQLite, chips fill-without-send, run button → onAgentDone → RunItem, origin='agent' badge via direct DB UPDATE + IPC-triggered repaint), +skill-cron 3 (fire-time template resolution, template-edit follow, delete → explicit failed job_runs with the dangling-skill error and the 技能已删除 pill), +gateway-quartet 2 (real-gateway GET /api/jobs|contents|accounts|skills shapes + POST-404 read-only boundary), cron-agent 400 group extended (prompt+skillId both-set, 6-field seconds — D6 strict caliber proven at the Gateway).
- Test-infrastructure findings fixed during R4: skills-page order contract (panel-level assertions are order-coupled within a spec sharing one app instance — now documented in-line at the chip test; worker-restart safety unaffected), shared-toast race (wait for the 3s toast to clear before asserting a new one), fire-and-forget disabled-state assertion withdrawn (millisecond transient by design — flaky; toast+done+RunItem anchors suffice).
- Docs sync (AC-S10): SECURITY.md #9 rewritten to current facts (two-server allowlist + file tools + hook-fenced bypassPermissions rationale) + #13 added (app tool surface grading, job_delete warn gates, bridge read-only boundary, skill prompt-injection trust chain); MCP.md bridge list + boundary note; DATABASE.md skills table + soft-reference semantics; CHANGELOG 0.5.0 entry.
- Gate: `npm run gate` exit 0 (ESLint 0 errors / 42 warnings — net -1 from cleaning a pre-existing dead variable; 4×typecheck, build, vitest 149, e2e 52). No new dependencies.
