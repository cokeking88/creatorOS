...
const IPC = {
  ...
  JOB_LIST: 'job:list', JOB_CREATE: 'job:create', JOB_TOGGLE: 'job:toggle', JOB_DELETE: 'job:delete',
  SKILL_CREATE: 'skill:create', SKILL_UPDATE: 'skill:update', SKILL_DELETE: 'skill:delete',
  LOGS_LIST: 'logs:list', LOGS_CLEAR: 'logs:clear',
  FILES_LIST: 'files:list', FILES_READ: 'files:read', FILES_WRITE: 'files:write', FILES_MKDIR: 'files:mkdir', FILES_RENAME: 'files:rename',
  ...
  AGENT_RUN: 'agent:run', AGENT_STOP: 'agent:stop', AGENT_RUNS_LIST: 'agent:runs.list',
...
contextBridge.exposeInMainWorld('creatorOS', {
  ...
  jobs:{list:()=>invoke(IPC.JOB_LIST),create:(x:unknown)=>invoke(IPC.JOB_CREATE,x),toggle:(id:string,e:boolean)=>invoke(IPC.JOB_TOGGLE,id,e),delete:(id:string)=>invoke(IPC.JOB_DELETE,id)},
  skills:{create:(x:{name:string;description?:string;promptTemplate:string})=>invoke(IPC.SKILL_CREATE,x),update:(id:string,x:{name?:string;description?:string;promptTemplate?:string})=>invoke(IPC.SKILL_UPDATE,id,x),delete:(id:string)=>invoke(IPC.SKILL_DELETE,id)},
  agent:{run:(p:string,resume?:string)=>invoke<{runId:string}>(IPC.AGENT_RUN,p,resume),stop:(runId:string)=>invoke<boolean>(IPC.AGENT_STOP,runId),runsList:(limit?:number)=>invoke<{agentRuns:{id:string;status:string;source:string;prompt:string;ok:boolean;costUsd:number|null;durationMs:number|null;startedAt:number;finishedAt:number|null}[];jobRuns:{id:string;jobId:string;jobName:string;status:string;startedAt:number;finishedAt:number|null;error:string|null}[]}>(IPC.AGENT_RUNS_LIST,limit??10)},
  logs:{list:(f?:unknown)=>invoke(IPC.LOGS_LIST,f),clear:()=>invoke(IPC.LOGS_CLEAR)},
  files:{list:(accountId:string)=>invoke(IPC.FILES_LIST,accountId),read:(accountId:string,rel:string)=>invoke(IPC.FILES_READ,accountId,rel),write:(accountId:string,rel:string,content:string)=>invoke(IPC.FILES_WRITE,accountId,rel,content),mkdir:(accountId:string,rel:string)=>invoke(IPC.FILES_MKDIR,accountId,rel),rename:(accountId:string,from:string,to:string)=>invoke(IPC.FILES_RENAME,accountId,from,to)},
  onFilesChanged:(cb:(p:{accountId:string;relPath:string|null})=>void)=>{const fn=(_e:any,p:{accountId:string;relPath:string|null})=>cb(p);ipcRenderer.on(IPC.EVENT_FILES_CHANGED,fn);return()=>ipcRenderer.removeListener(IPC.EVENT_FILES_CHANGED,fn);},
